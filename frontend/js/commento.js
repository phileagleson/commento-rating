(function(global, document) {
  ...
  var ID_RATINGS_CONTAINER = "ratings";
  var ID_RATING_TOTAL_SPAN = "rating-total-span"
  var ID_STAR_VALUE = 'star-value';
  var NUM_STARS = 5;
  var EMPTY_STAR = '&#9734;';
  var FILLED_STAR = '&#9733;'
  ...
  function textareaCreate(id, edit) {
    ...
    var ratingsContainer = create("div");
    var starEl = create("button");
    starEl.classList.add("star");
    for (var x = 0; x < NUM_STARS; x++) {
      var newStar = starEl.cloneNode()
      newStar.innerHTML = EMPTY_STAR;
      ratingsContainer.appendChild(newStar);
    }
    var starValueEl = create("input");
    starValueEl.type = 'hidden';
    starValueEl.id = ID_STAR_VALUE;
    starValueEl.value = 0;


    ratingsContainer.id = ID_RATINGS_CONTAINER;
    ...
    classAdd(ratingsContainer, "ratings");
    ...
    if (id === 'root') {
      append(textareaSuperContainer, ratingsContainer);
      append(textareaSuperContainer, starValueEl);
    }
  ...
  function rootCreate(callback) {
    ...
    allStars.forEach((star, currIdx) => {
      star.addEventListener('click', () => {
        const currentStar = currIdx + 1;
        const starValueEl = document.querySelector('#star-value');
        starValueEl.value = currentStar;

        allStars.forEach((star, j) => {
          if (currentStar >= j + 1) {
            star.innerHTML = FILLED_STAR;
          } else {
            star.innerHTML = EMPTY_STAR;
          }
        })
      })
    })
  ...
  global.commentNew = function(id, commenterToken, callback) {
    ...
    if (id === 'root') {
      const rating = document.querySelector('#star-value').value;
      if (rating && +rating > 0)
        json["rating"] = rating;
    }
  ...
  function commentsRecurse(parentMap, parentHex) {
    ...
    cur.forEach(function(comment) {
      ...
      var rating = create("div")
      var star = create("button");
      ...
      if (comment.rating) {
        for (var x = 0; x < NUM_STARS; x++) {
          ...
          if (comment.rating >= x + 1)
            newStar.innerHTML = FILLED_STAR;
          else
            newStar.innerHTML = EMPTY_STAR;

          append(rating, newStar);
        }
      }
      ...
      classAdd(name, "name");
      classAdd(rating, "comment-rating");
  ...
  async function getRatings() {
    const domain = parent.location.host
    const jsonData = {
      domain,
      path: pageId
    }
